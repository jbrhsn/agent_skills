...
Applies a "lazy senior developer" discipline to every coding task: the smallest *correct and clear* solution, reusing what exists, never over-engineering. Its shared rules — the writing-the-code judgment order, the never-cut non-negotiables, comment/test conventions, Python `uv` tooling, and `lazy:` deferral markers — are defined once and referenced by every workflow unit rather than restated.
...
- **Minimal-vs-robust — hand back and ask.** On a genuine tradeoff between a minimal and a more robust/extensible version (not a non-negotiable, a real judgment call), it states both options briefly and hands the decision back to the user rather than silently picking one. It does not raise this gate for trivial cases.
...
- **Self-verification before reporting.** Every unit that changes something verifies its own work first — running tests/build/lint if they exist, else inspection — and hands back a terse structured report contract (files changed / what was verified / pass-fail), mirroring an executor's discipline.
...
The process is expressed as self-contained units, each with a goal/scope, inputs, what to do, a self-verify step, and a terse report contract. Mandatory pauses are explicit **STOP GATE (hand back)** points where control returns to the user/orchestrator.

### BUILD path — normal coding work (default)

| Unit | Goal | Hand-back? |
|---|---|---|
| **B1 — Read before writing** | Scoped exploration (or delegate to an `explore` subagent) to learn conventions and find existing equivalents before touching code. | — |
| **B2 — Decide the approach** | Apply the judgment order and non-negotiables to pick the smallest correct approach. | **STOP GATE** on a genuine minimal-vs-robust tradeoff only (not trivial cases). |
| **B3 — Write the code** | Implement the smallest *correct and clear* solution; keep non-negotiables in; mark deliberate deferrals with `lazy:`. Self-verify by running tests/build/lint (or inspection) before reporting. | — |
| **B4 — Report impact** | Report measured size (`git diff --stat` / `wc -l`) for non-trivial work; skipped for trivial one-line fixes. | — |

### REVIEW/AUDIT path — explicitly invoked

Both units delegate the read-heavy pass to subagents and **return findings only — they never auto-fix.** Each ends in a STOP GATE handing the cut/prioritization decisions back to the user. Detailed steps live in `references/workflows.md`; the audit's what-to-look-for and what-NOT-to-flag rules live in `references/audit-checklist.md`.

| Workflow | What it does |
|---|---|
| **`/review-diff`** (Unit A1) | Confirms the baseline (`git diff`, `git diff --staged`, `git diff <base>`, or the files changed this session), then delegates a fresh-eyes review to a `general` subagent that flags over-engineering only — reinvented stdlib, unnecessary deps, speculative abstractions, dead flexibility, needless verbosity. Relays a short scannable list with `file:line`, what to cut, and what replaces it, then hands back for the user's cut decisions. |
| **`/audit-repo`** (Unit A2) | Delegates a repo-wide scan to subagents (briefed with `references/audit-checklist.md`), excluding vendored/generated dirs and prioritizing the largest, most-depended-on source files. Consolidates a ranked list (biggest wins first) of what to delete, simplify, or replace, each with a one-line rationale and rough impact, then hands back for the user's prioritization. |
...
- **Hands back and asks rather than silently choosing** when a minimal-vs-robust tradeoff is genuinely debatable (Unit B2's STOP GATE).
