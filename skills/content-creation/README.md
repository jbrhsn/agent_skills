...
Both writers stop before drafting and make you approve the angle — Medium writes a
`medium_brief.md`, LinkedIn shows the claim, the surviving hooks, and what it proposes
to leave out. The gate exists because both skills compress hard, and neither should
get to decide on its own which of your experiences does not survive the compression.
Whatever is dropped is named before it is dropped, and both writers carry a standing
rule against cutting something you lived through without asking.

Neither writer scores a finished piece. The audit passes are rewrite triggers inside
the workflow, not a grade — a number on a draft you just wrote is self-marking.

What is platform-specific is the advice itself: LinkedIn's dwell time, saves, and
substantive comments behave nothing like Medium's read ratio and distribution rules.
That is the point of splitting them rather than shipping one "write a post" skill.
