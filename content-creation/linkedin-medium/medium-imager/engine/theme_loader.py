#!/usr/bin/env python3
"""Load and validate Medium-imager theme token files.

A theme is pure data (JSON): color tokens, font stack, decoration flags, and
a Pygments syntax-highlight style. Themes can be referenced by built-in name
(a file in themes/) or by an absolute or cwd-relative path to a custom theme JSON.

Validation is intentionally light (themes are user-editable): required keys must
be present and colors must be parseable hex. Contrast is checked separately and
reported as a warning, never a hard failure.

Stdlib only (contrast_check is a local module).
"""
...
def _validate(data: dict, path: str) -> None:
    ...
    # pygments_style is optional but recommended
    pygments_style = data.get("pygments_style")
    if pygments_style and not isinstance(pygments_style, str):
        raise ThemeError(f"theme {path}: 'pygments_style' must be a string, got {type(pygments_style)}")
