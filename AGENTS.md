...
- Run Python through `uv run` using the target project's local `.venv`; do not rely on a global interpreter, an active parent environment, or an inline-metadata script environment. Save newly authored executable programs under the target project's `.temp/` and invoke the saved file rather than passing source through the terminal. Reuse an existing script first; promote maintained project tooling into its `scripts/` directory. This applies to Python, Node/TypeScript, shell, and other interpreters, while ordinary one-command tool invocations remain direct.
